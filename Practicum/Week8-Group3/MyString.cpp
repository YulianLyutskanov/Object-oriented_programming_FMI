...
static unsigned roundToPowerOfTwo(unsigned v) // static! -> will be discussed in the next lecture
{
    v--;
    v |= v >> 1;
    v |= v >> 2;
...
MyString &MyString::operator+=(const MyString &other)
{
    ...
    // use strncat instead of strcat, because strcat will not work for str += str (the term zero of str will be
    // destroyed by the first char)
